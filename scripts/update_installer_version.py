...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=ReversEz Demo Installer\n"
      else:
        line="OutputBaseFilename=ReversEz Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the ReversEz Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the ReversEz installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=ReversEz Demo installer\n"
     else:
       line="SetupWindowTitle=ReversEz installer\n"
