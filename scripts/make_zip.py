...
def  main():
  ...
  installer = "\installer\ReversEz Installer.exe"
   
  if demo:
    installer = "\installer\ReversEz Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\ReversEz manual.pdf" 
  ]
  ...
  ZIPNAME = "ReversEz-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "ReversEz-v" + FULLVERSIONSTR + "-win-demo.zip"
